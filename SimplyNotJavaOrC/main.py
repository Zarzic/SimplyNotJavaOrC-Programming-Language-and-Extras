...
"""
from ParserSimplyNotJavaOrC import Parser, ParseType, ParseTree
...
def eval_read(t, env):
  """
    ...
    """
  # Correctly casts the correct type when given a value based on input. Type checking is then done when combining any types by the interpreter. However no string functions exist so arithmetic operations are only
  # on ints and floats. Therfore, anything that is a int or float is cast correctly.
...
def eval_assign(t, env):
  if t.children[0].node_type == ParseType.ARRAY:
     v2 = t.children[0].children[0].token.lexeme  # this is the array name
...
def eval_add(t, env):  # All math functions now work with all type checking
  """
...
def eval_call(t, env):  # Test to see if this works with and without parameters
  """
    Call a function
    """
...
def eval_import(t, env):
  ...
  ##### Have to implement file functions to make this work
...
def eval_split(t, env):
  ...
  # implicit array, fixed boundaries, and such figure out some way for how long it is


def eval_block(t, env):  # just like statement list (do for i in ___)
...
def eval_paramlist(t, env):
  fun_result = []
  for c in t.children:
    fun_result.insert(0, c.children[0].token.lexeme)
...
def eval_swap(t, env):
  ...
  if (not value.node_type == ParseType.REF) or (not value2.node_type == ParseType.REF):
    print("Invalid swap operation (proper syntax: [REF] :=: [REF2])")
    sys.exit(-1)

  name1 = value.children[0].token.lexeme
  name1_val = env.lookup(name1)

  name2 = value2.children[0].token.lexeme
  name2_val = env.lookup(name2)

  (name1_val.ref_value, name2_val.ref_value) = (name2_val.ref_value, name1_val.ref_value)
# CAN EXPAND FOR ARRAYS IN THE FURTURE
