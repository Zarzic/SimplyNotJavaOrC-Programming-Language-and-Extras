...
from SimplyNotJavaOrCLexer2 import Token, Lexer
...
# FIX MULTIPLE FUNCTION DECLARATIONS WITH AND WITHOUT PARAMATERS. NO VARS BEFORE IT. = DONE
# Single Functions:
# Without parameters = DONE
# with paramaters = DONE

# Multi-Functions:
# Without Parameters = DONE
# With Parameters = DONE
###########################################################################################

# FIX IF STARTS WITH VAR THEN INTO FUNCTION DECLARATIONS WITH AND WITHOUT PARAMATERS.
# SINGLE FUNCTION AND SINGLE VAR WITH NO PARAMATERS = DONE
# SINGLE FUNCTION AND SINGLE VAR WITH PARAMATERS = DONE
# FUNCTIONS AND VARS WITH NO PARAMATERS: = DONE
# FUNCTIONS AND VARS WITH PARAMATERS = DONE

###########################################################################################

# CHECK ARR WITH AND WITHOUT STATEMENTS = SHOULD BE DONE
# CHECK EACH STATEMENT WITH AND WITHOUT MULTIPLE (ONLY SINGLE IS DONE BUT SHOULD WORK WITH MULTIPLE)
# CHECK THAT REF TO ALL THE VARS IN DEC AND INSIDE BLOCK WORK = SHOULD BE DONE
# IMPORT CHANGED TO ABLE TO INCLUDE REFERENCE NOW, MAKE EASIER TO GET I/O FROM USER

# ONLY SINGLE STATEMENTS:
# WORKS: Add, Sub, Mul, Div, Power, NUMBER, CHARACTER, STRING, Printing in quotes, Parens in Math Functions,
# IF, BOOL, BEGIN, END, Printing Vars, WHILE

# IMPORT NOW RETURNS TO A VAR, BUT I DON'T KNOW IF THE TYPE IS RIGHT FOR THE VAR. THIS MEANS WE NO LONGER NEED THE "AS ID" AREA.
# THIS WAS CHANGED

###########################################################################################
# 11/28/2022 CHANGES:
# ADDED THE STRING TYPE AND THE STRINGLIT FOR THE PARSER AND LEXER.
...
##########################################################################################
# IMPORT AND SPLIT WORK BUT RETURN TO A VAR THAT HAS NO METHODS MADE FOR IT.
# THEREFORE FURTURE WORK CAN BE DONE TO FIX THIS PROBLEM.
...
class Parser:
  ...
  ###########
  # From here on down, everything is SimplyNotJavaOrCLexer specific
  ###########
  ...
  def __stmnt(self):
    if self.__has(Token.ID):
      ...
      #leaf = self.__lexer.get_tok()
      #self.__next()
      #result = ParseTree(ParseType.ATOMIC, leaf)
      #result.children.append(self.__factor2())
      #result.children.append(self.__term2())
      #result.children.append(self.__expression2())
      #return result
