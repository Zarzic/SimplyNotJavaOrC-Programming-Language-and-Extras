...
#############################################################################################
def a_mul_goal(graph, value):
  q = PriorityQueue()
  queue = deque((graph.find_node(value).md, [(graph.find_node(value), [graph.find_node(value).value])]))
  visited = set()
  depth = 0
  max_fringe = 0
  max_expansion = 0
  ...
  while queue:
    ...
    md, pathposition = queue.popleft()
    visited.add(pathposition[0])
    if (pathposition[0].value in list_of_points):
        visited_1 = pathposition[1]
        ...
        for bb in reversed_visited:
            ...
            cc = tuple(y)
            list_list.append(cc)
        list_of_points.remove(pathposition[0].value)
        for yyy in list_list:
            list_list_yeah.add(yyy)
        if (len(list_of_points) == 0):
            return depth, max_fringe, max_expansion
    max_expansion = max_expansion + 1
    for element in pathposition[0].neighbors:
        if element not in visited:
          new_path = pathposition[1] + [element.value]
          queue.put(( , (element, new_path)))
    depth = depth + 1
  return None
#############################################################################################
...
ans1 = a_mul_goal(g1, (pac_y, pac_x))
...
for ir, row in enumerate(changed1):
  for ic, col in enumerate(row):
    ...
    if changed1[ir][ic] == 'P':
      ...
      changed1[ir][ic] = '~'
changed1.reverse()
print("Graphical Representation of A* Search for Multi-Goal:")
print("")
for xx in changed1:
  print(xx)